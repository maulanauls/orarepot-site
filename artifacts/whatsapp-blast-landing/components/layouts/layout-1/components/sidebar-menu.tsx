...
export function SidebarMenu() {
  ...
  const labelFor = useCallback(
    (item: MenuItem) => {
      ...
      if (item.path && MENU_I18N[item.path]) {
        return t(MENU_I18N[item.path]);
      }
      return item.title ?? '';
    },
    [t],
  ...
  const matchPath = useCallback(
    (path: string): boolean => {
      ...
      // Keep Template OTP selected on detail routes only
      if (
        path === '/dashboard/otp/templates' &&
        pathname.startsWith('/dashboard/otp/templates/')
      ) {
        return true;
      }
      return false;
    ...
    if (item.children) {
      ...
      return (
        <AccordionMenuItem
          ...
          className="text-[13px]"
        >
          <Link href={item.path || '#'}>{labelFor(item)}</Link>
        </AccordionMenuItem>
      );
    }
  };
